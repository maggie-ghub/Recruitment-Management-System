<template>
  <div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-2xl border border-[#f0e9dc] flex flex-wrap items-center gap-4">
      <label class="text-xs font-bold uppercase tracking-wider text-[#7a6e5a]">Filter by Company:</label>
      <select
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
      <div class="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-[#f0e9dc]">
        ...
        <form @submit.prevent="handleSubmit" class="mt-4 space-y-4">
          <div>
            <label class="block text-xs font-semibold uppercase tracking-wider text-[#7a6e5a] mb-1">Company *</label>
            <select
          ...
          <div>
            <label class="block text-xs font-semibold uppercase tracking-wider text-[#7a6e5a] mb-1">Department Name *</label>
            <input
              v-model="form.name"
              type="text"
              required
